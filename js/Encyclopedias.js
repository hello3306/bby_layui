...
layui.use('table', function () {
    ...
    table.render({
        elem: '#test'
        , url: baseUrl+'/knowledge/getEncyclopedias'
        , toolbar: '#toolbarDemo'
        , title: '用户数据表'
        , headers: {token: token}
        , request: {
        ...
        , cols: [[
            ...
            , {field: 'name', title: '类别',templet: '#usernameTpl', width: 120}
            , {field: 'description', title: '内容', width: 1100}
        ...
        switch (obj.event) {
            case 'getCheckData':
                layer.open({
                    type: 2,
                    area: ['1200px', '600px'],
                    content: 'add_Encyclopedias.html',
                });
    ...
    table.on('tool(test)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                ...
                layer.close(index);
                let param = {
                    url: '/knowledge/deleteEncyclopedias',
                    type: 'post',
                    data: {
                        id: data.id
                    },
            ...
            layer.open({
                type: 2,
                area: ['1200px', '600px'],
                content: 'add_Encyclopedias.html'+'?id='+data.id,
            });
        }
    });
});