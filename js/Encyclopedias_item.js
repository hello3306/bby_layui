...
layui.use('table', function () {
    ...
    table.render({
        elem: '#test'
        , url: baseUrl+'/knowledge/getEncyclopediasItem'
        , toolbar: '#toolbarDemo'
        , title: '用户数据表'
        , headers: {token: token}
        ,where:{
        ...
        switch (obj.event) {
            case 'getCheckData':
                layer.open({
                    type: 2,
                    area: ['1200px', '600px'],
                    content: 'add_Encyclopedias_item.html'+'?code='+id,
                });
    ...
    table.on('tool(test)', function (obj) {
        ...
        if (obj.event === 'del') {
            layer.confirm('真的删除行么', function (index) {
                ...
                layer.close(index);
                let param = {
                    url: '/knowledge/deleteEncyclopediasItem',
                    type: 'post',
                    data: {
                        id: data.id
                    },
            ...
            layer.open({
                type: 2,
                area: ['1200px', '600px'],
                content: 'add_Encyclopedias_item.html'+'?code='+id+'&id='+data.id,
            });
        }
    });
});